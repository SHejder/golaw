...
class Kama_Contents
{
    ...
    function make_contents(& $content, $tags = '')
    {
        ...
        if (isset($this->temp->as_table)) {
            ...
            $contents =
                ($_is_tit ? '<div class="article__content-list"' . $ItemList . ' >' : '') .
                ($_is_tit ? '<p class="article__cl-title"' . ($ItemList ? ' itemprop="name"' : '') . '>' . $_tit . '</p>' . "\n" : '') .
                '<ol class="article__cl-wrap"' . ((!$_tit || $embed) ? ' id="kcmenu"' : '') . (($ItemList && !$_is_tit) ? $ItemList : '') . '>' . "\n" .
                implode('', $this->contents) .
                '</ol>' . "\n" .
                ($_is_tit ? '</div>' : '');
        }
    ...
    private function _make_contents_callback($match)
    {
        ...
        if ($level > 0)
            $sub = ($opt->margin ? ' style="margin-left:' . ($level * $opt->margin) . 'px;"' : '') . ' class="article__cl-item"';
        else
            $sub = ' class="top"';
        ...
        if (isset($this->temp->as_table)) {
            ...
            $this->contents[] = "\t" . '
				...
				<span class="article__cl-num cl-num"></span>
					<a  class="article__cl-link achor-atc" rel="nofollow"' . ($_is_mark ? ' itemprop="url"' : '') . ' href="' . $opt->page_url . '#' . $anchor . '">
						' . ($_is_mark ? '<span itemprop="name">' . $cont_elem_txt . '</span>' : '<span>'.$cont_elem_txt.'</span>') . '
					</a>
					' . ($_is_mark ? ' <meta itemprop="position" content="' . $temp->counter . '" />' : '') . '
				</li>' . "\n";
        }
